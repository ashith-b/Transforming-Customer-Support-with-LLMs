...
def display_response(query, df, index):
    ...
    distances, indices = semantic_similarity(
        query, index, model="text-embedding-3-small"
    )
    ...
    st.write(
        f"Following responses will be generated:\n"
...
if "llm_response" in st.session_state:
    ...
    feedback = st.text_area(
        "Provide feedback or additional instructions for the response:", key="feedback"
    )
    ...
    if st.button("Regenerate Response"):
        if feedback:
            ...
            new_llm_response = call_llm(
                new_query, top_similar_instructions["response"].tolist()
            )
