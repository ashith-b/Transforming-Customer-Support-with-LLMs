...
def test_project_structure():
    ...
    required_files = ["src/helper.py", "demo.py", "requirements.txt", "README.md"]
...
def test_required_folders():
    ...
    required_folders = ["src", "tests", "Customer_Support_Training_Dataset"]
...
def test_demo_file_structure():
    ...
    assert (
        "st.title" in content or "st.header" in content
...
def test_helper_has_functions():
    ...
    assert (
        "call_llm" in content or "openai" in content.lower()
...
def test_llm_function_exists():
    ...
    try:
        ...
        assert (
            "def call_llm" in content or "def " in content
...
def test_vector_store_exists():
    ...
    if os.path.exists(vector_store_path):
        ...
        assert os.path.getsize(vector_store_path) > 0, "FAISS index should not be empty"
    else:
...
def test_dataset_exists():
    ...
    dataset_path = (
        "Customer_Support_Training_Dataset/Customer_Support_Training_Dataset.csv"
    )

    if os.path.exists(dataset_path):
        assert os.path.getsize(dataset_path) > 0, "Dataset should not be empty"
    else:
