...
def create_embeddings(df: pd.DataFrame, column_name: str, model: str) -> np.ndarray:
    ...
    df[f"{column_name}_vector"] = df[column_name].apply(
...
def semantic_similarity(
    query: str, index: faiss.Index, model: str, k: int = 3
...
def call_llm(query: str, responses: List[str]) -> str:
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a helpful assistant and help answer customer's query and request. Answer on the basis of provided context only.",
        },
        {
            "role": "user",
            "content": f"""On the basis of the input customer query determine or suggest the following things about the input query: {query}:
